...
public class SimpleItemItemModelProvider implements Provider<SimpleItemItemModel> {
    ...
    public SimpleItemItemModel get() {
        ...
        // TODO Compute the similarities between each pair of items
            ...
            for (long OuterItem : itemVectors.keySet()) {
                //long OuterItemId = OuterItem.getKey();
               // List<Rating> OuterItemRatings = itemVectors.get(OuterItem);
                Long2DoubleOpenHashMap Similarity = new Long2DoubleOpenHashMap();
                for (long InnerItem : itemVectors.keySet()) {
                    //long InnerItemId = InnerItem.getKey();
                    //List<Rating> InnerItemRatings = InnerItem.getValue();
                    if(InnerItem != OuterItem) {
                        Long2DoubleOpenHashMap OuterItemRatingsMap = new Long2DoubleOpenHashMap((itemVectors.get(OuterItem)));
                        Long2DoubleOpenHashMap InnerItemRatingsMap = new Long2DoubleOpenHashMap((itemVectors.get(InnerItem)));
                        double sim = Vectors.dotProduct(OuterItemRatingsMap, InnerItemRatingsMap) / (Vectors.euclideanNorm(OuterItemRatingsMap) * Vectors.euclideanNorm(InnerItemRatingsMap));

                        if (sim > 0 ) {
                            Similarity.put(InnerItem, sim);
                        }
                       /* else {
                            Similarity.put(InnerItem, 0.0);
                        }*/
                    }
                }
                itemSimilarities.put(OuterItem,Similarity);
                //Similarity.clear();
            }
