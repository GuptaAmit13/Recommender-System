...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            for(int row=userIndex.getLowerBound();row<userIndex.getUpperBound();row++)
            {
                for(int col=itemIndex.getLowerBound();col<itemIndex.getUpperBound();col++)
                {
                    matrix.addToEntry(row,col,0.0);
                }
            }
            ...
            for(Rating r:ratings)
            {
                ...
                double baseline = this.baseline.getIntercept()+this.baseline.getUserBias(userID)+this.baseline.getItemBias(itemID);
                matrix.addToEntry(userIndex.getIndex(userID),itemIndex.getIndex(itemID),val-baseline);
            }
