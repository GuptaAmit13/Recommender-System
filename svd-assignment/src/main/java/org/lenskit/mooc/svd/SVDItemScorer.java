...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        RealMatrix userMatrix = model.getUserFeatureMatrix();

       // RealMatrix itemMatrix = model.getItemFeatureMatrix();
        RealVector weights = model.getFeatureWeights();

        RealVector userAndWeights=userFeatures.ebeMultiply(weights);
        // TODO Compute the predictions
        for(Long item:items)
        {
            RealVector itemFeatures=model.getItemVector(item);
            double baseline = this.baseline.getIntercept()+this.baseline.getUserBias(user)+this.baseline.getItemBias(item);
            double pred=itemFeatures.dotProduct(userAndWeights);
            pred=pred+baseline;
