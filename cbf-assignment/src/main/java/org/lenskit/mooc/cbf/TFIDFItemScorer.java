...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        Map<String,Double> temp=model.getItemVector(2231);

        for (Long item: items) {
            Map<String, Double> iv = model.getItemVector(item);
            Double numerator=0.0;
            Double denominatorQ=0.0;
            Double denominatorP=0.0;
            ...
            Result r;
            boolean flag=false;
            for(String s:userVector.keySet())
            {
                if(iv.get(s)!=null && userVector.get(s)!=null) {
                    flag=true;
                    numerator += iv.get(s) * userVector.get(s);
                }
                if(userVector.get(s)!=null)
                    denominatorP+=userVector.get(s)*userVector.get(s);
            }
            for(String s:iv.keySet())
            {
                if(iv.get(s)!=null)
                    denominatorQ+=iv.get(s)*iv.get(s);
            }
            Double cosineSimilarity=0.0;
            if(flag)
                cosineSimilarity = numerator / (Math.sqrt(denominatorQ) * Math.sqrt(denominatorP));
          //  if(cosineSimilarity==1)cosineSimilarity=0.0; //NEED TO REMOVE LATER
            r = Results.create(item, cosineSimilarity);
            results.add(r);
