...
public class LiftAssociationModelProvider implements Provider<AssociationModel> {
    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            Integer ysize = new Integer(0);
            Integer xsize = new Integer(0);
            Double y;
            Double x;
            Long ymovie = new Long(0);
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry : itemUsers.long2ObjectEntrySet()) {
                ...
                ArrayList<Long> zUsers = new ArrayList<>(yUsers);
                Double zSize = new Double(yUsers.size());
                if (yId != xId) {
                    flag = false;
                    try {
                        zUsers.retainAll(xUsers);
                        ysize = zUsers.size();
                        y=ysize.doubleValue();
                        xsize = xUsers.size();
                        x=xsize.doubleValue();
                        ymovie=yId;
                        Double assc=new Double(((y/allUsers.size())/((x/allUsers.size())*(zSize/allUsers.size()))));
                        itemScores.put(ymovie,assc);
                    } catch (Exception e) {
