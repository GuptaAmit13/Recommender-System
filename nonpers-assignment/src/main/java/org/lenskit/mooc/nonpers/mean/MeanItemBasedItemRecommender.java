...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        HashMap <Long,Double> sortedMean = new HashMap<>();
        Iterator<Long> it =  items.iterator();

        //Adding the mean ratings calculated in the provider class
        while(it.hasNext()){
            Long mmovi = new Long(it.next());
            sortedMean.put(mmovi,model.getMeanRating(mmovi));
        }


        //Sorting the sortedMap HashMap
        List<Long> mapKeys = new ArrayList<>(sortedMean.keySet());
        List<Double> mapValues = new ArrayList<>(sortedMean.values());
        ...
        while (valueIt.hasNext()) {
            ...
            while (keyIt.hasNext()) {
                Long key = keyIt.next();
                Double comp1 = sortedMean.get(key);
                Double comp2 = val;
        ...
        //Adding the top N items to the Result ArrayList
        int i=0;
